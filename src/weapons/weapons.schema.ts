...
export const weaponsCreateSchema = Joi.object<WeaponsCreateSchema>({
  ...
  strength: Joi.number().required().min(1).max(200),
  bullets_req: Joi.number().required().min(1).max(300)
});

export const weaponsUpdateSchema = Joi.object<Partial<WeaponsCreateSchema>>({
  name: Joi.string().optional().min(3).max(100),
  strength: Joi.number().optional().min(1).max(200),
  bullets_req: Joi.number().optional().min(1).max(300)
});
