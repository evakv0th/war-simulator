...
export const armiesCreateSchema = Joi.object<ArmiesCreateSchema>({
  ...
  fuel_amount: Joi.number().required().min(0).max(1000),
  bullets_amount: Joi.number().required().min(0).max(1000),
});
...
export const armiesUpdateSchema = Joi.object<Partial<ArmiesCreateSchema>>({
    ...
    fuel_amount: Joi.number().optional().min(0).max(1000),
    bullets_amount: Joi.number().optional().min(0).max(1000),
});
